module Diplomacy
  class OrderParser
    ...
    def parse_single_order(orderblob)
      ...
      if /^[AF](?'unit_area'\w{3})(?'unit_area_coast'\(.+?\))?S[AF](?'dst'\w{3})(?'dst_coast'\(.+?\))?$/ =~ orderblob
        unit = @gamestate[unit_area.to_sym].unit unless unit_area.nil?
      ...
      raise OrderParsingError.new(orderblob), 'order didn\'t match any order type'
    end
    ...
    def parse_single_build(orderblob)
      if /^(?'unit_type'[AF])(?'unit_area'\w{3})(?'build'[BD])$/ =~ orderblob
      	is_build = (build == 'B')
        is_army = (unit_type == 'A')
      	if is_build
